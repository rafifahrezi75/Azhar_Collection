...
import { getLayananList, getCompanySettings } from '../firebase/adminService'
...
export default function Footer({
  ...
  useEffect(() => {
    let isMounted = true
    Promise.all([getLayananList(), getCompanySettings()]).then(([list, compSettings]) => {
      if (isMounted) {
        if (list) setServices(list.slice(0, 5))
        if (compSettings) {
          setInfo({
            ...companyInfo,
            ...compSettings,
    ...
    <footer className="footer-wrapper">
      ...
      <div className="container">
        <div className="footer-grid">
          <div>
            ...
            <p style={{ fontSize: '0.84375rem', color: 'rgba(255, 255, 255, 0.72)', lineHeight: 1.6, marginBottom: '1.5rem' }}>
              {info.tagline || 'Produsen konveksi dan garment terpercaya di Sidoarjo. Melayani pesanan kustom seragam sekolah, batik identitas, kemeja PDH/PDL, jas almamater, dan pakaian dinas dengan mutu jahitan rapi bergaransi.'}
            </p>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', fontSize: '0.8125rem', color: 'rgba(255, 255, 255, 0.75)' }}>
              ...
              <div style={{ display: 'flex', gap: '0.625rem', alignItems: 'center' }}>
                ...
                <span>{info.phonePrimary || info.phone} ({info.contactPersonPrimary || 'Ach. Haris'})</span>
              </div>
              {info.phoneSecondary && (
                <div style={{ display: 'flex', gap: '0.625rem', alignItems: 'center' }}>
                  <Phone size={16} style={{ color: 'var(--color-accent)', flexShrink: 0 }} />
                  <span>{info.phoneSecondary} ({info.contactPersonSecondary || 'Lazuardi'})</span>
                </div>
              )}
              <div style={{ display: 'flex', gap: '0.625rem', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: '0.625rem', alignItems: 'center' }}>
                ...
                <span>{info.workingHours || info.hours}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="footer-bottom">
          <div>
            &copy; {new Date().getFullYear()} {info.name || 'Azhar Collection'}. Seluruh hak cipta dilindungi.
          </div>
